...
class RemandAction extends Action {
	/**
	 * 项目需求列表
	 *@param $pid  传入所属项目ID
...
//			var_dump($result->getLastSql());exit;
//			dump($info);exit;
			$new_info = array();
			//$items = array();
	...
	public function add($act=NULL,$go=false,$id=NULL,$pid=NULL){
		...
		//main
		$Remand = D('Report_table');
		$Remand_baseinfo = D('Report_baseinfo_table');
		$userid = $_SESSION['login']['se_id'];
		$username = $_SESSION['login']['se_user'];
		if($go==false){
			$this->assign('pid',$pid);
			$this->assign('uniqid',uniqid());
			if($act=='add'){
				$reno = $Remand->where('style = 49')->Max('bugno');

				//TODO 暂定编号为数字类型 以后需要改则变更
				$reno = intval(substr($reno,2));
				$reno = $reno+1;
				$reno = 'RE'.str_pad($reno,8,'0',STR_PAD_LEFT);
				$this->assign('bugno',$reno);
				...
				if(!is_int((int)$id)){
					...
					$map['id'] = array('eq',$id);
					$info = $Remand->relation(array('baseinfo','create'))->where($map)->find();
					//dump($info);
					$bugno = 'RE'.str_pad($info['id'],8,'0',STR_PAD_LEFT);
			...
			if($act=='add'){
				...
				$data['uptime'] = $nowtime;
				$add = $Remand->relation(true)->add($data);
				if($add>0){
				...
				if(!is_int((int)$id)){
					...
					if($role!='all' && !in_array('a',$role) ){
						$user_id = $Remand->where('id='.$id)->getField('creator');
						if($userid!=$user_id){
							echo 2; exit;
						}
					}
					$data['uptime'] = $nowtime;
					$map['id'] = array('eq',$id);
					$olduser = $Remand->where($map)->getField('user_id');
					$edit = $Remand->relation(true)->where($map)->save($data);
					unset($map);
					if($edit !== false){
